...
function gridwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('gridwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'gridwp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'gridwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'gridwp_main_options_panel';
    ...
    gridwp_getting_started($wp_customize);
    gridwp_header_options($wp_customize);
    gridwp_post_options($wp_customize);
    gridwp_social_profiles($wp_customize);
    gridwp_footer_options($wp_customize);
    gridwp_other_options($wp_customize);
    gridwp_recomm_plugin_options($wp_customize);
    gridwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'gridwp_register_theme_customizer' );

function gridwp_customizer_js_scripts() {    
    wp_enqueue_script('gridwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'gridwp_customizer_js_scripts' );